  ...
  return {
    ...
    info: {
      ...
      description: [
        ...
        '',
        `**Two doors, one ledger.** Partner API: create an account (dashboard or \`POST /v1/partners\`) and send your single API key as \`X-API-Key\`. SEP door for wallets: SEP-1 discovery via \`/.well-known/stellar.toml\`, SEP-10 auth at \`/auth\`, SEP-6 at \`/sep6\`, SEP-12 at \`/sep12\`, SEP-38 at \`/sep38\`. Rates come from Reflector's USD/TRY oracle on Stellar mainnet (fallback: static), with a ${cfg.spreadBps} bps spread each side.`,
      ].join('\n'),
    ...
    tags: [
      ...
      { name: 'SEP-10', description: 'Stellar web authentication for wallets. Produces the bearer JWT used by the SEP endpoints.' },
      { name: 'SEP-6', description: 'Programmatic deposit/withdraw for wallets (TRANSFER_SERVER). Same ledger and treasury as the partner API.' },
      { name: 'SEP-12', description: 'KYC for wallets. Simulated: NEEDS_INFO until any PUT, then ACCEPTED. No personal data is required or stored beyond optional name/email/IBAN.' },
      { name: 'SEP-38', description: 'Quotes for wallets: iso4217:TRY <-> stellar:USDC:<issuer>.' },
    ],
    paths: {
      ...
      '/auth': {
        get: {
          tags: ['SEP-10'], security: [], summary: 'SEP-10 challenge',
          parameters: [
            { name: 'account', in: 'query', required: true, schema: { type: 'string' }, description: 'G... or M... account to authenticate' },
            { name: 'memo', in: 'query', schema: { type: 'string' }, description: 'Optional id memo for shared accounts' },
            { name: 'home_domain', in: 'query', schema: { type: 'string' } },
            { name: 'client_domain', in: 'query', schema: { type: 'string' } },
          ],
          responses: { 200: resp('Challenge', { type: 'object', properties: { transaction: { type: 'string', description: 'base64 XDR, sign with the account key' }, network_passphrase: { type: 'string' } } }), 400: resp('Invalid account / memo / domains', { type: 'object', properties: { error: { type: 'string' } } }) },
        },
        post: {
          tags: ['SEP-10'], security: [], summary: 'Exchange the signed challenge for a JWT',
          requestBody: json({ type: 'object', required: ['transaction'], properties: { transaction: { type: 'string' } } }),
          responses: { 200: resp('Token', { type: 'object', properties: { token: { type: 'string' } } }), 400: resp('Verification failed', { type: 'object', properties: { error: { type: 'string' } } }) },
        },
      },
      '/sep6/info': { get: { tags: ['SEP-6'], security: [], summary: 'Capabilities per asset (deposit, withdraw, exchange variants, features)', responses: { 200: resp('Info') } } },
      '/sep6/deposit': {
        get: {
          tags: ['SEP-6'], security: [{ Sep10Jwt: [] }], summary: 'Open a deposit: bank details + reference',
          description: 'Creates a transaction in `pending_user_transfer_start` and returns SEP-9 `instructions` (bank_name, bank_account_number = IBAN, external_transfer_memo = reference). The bank is simulated: open `more_info_url` and press the button, or `POST /sep6/tx/{id}/simulate-bank-transfer`. Real testnet USDC is then paid to `account` (payment, or claimable balance when there is no trustline).',
          parameters: [
            { name: 'asset_code', in: 'query', required: true, schema: { type: 'string', example: 'USDC' } },
            { name: 'account', in: 'query', schema: { type: 'string' }, description: 'Destination; defaults to the token subject' },
            { name: 'funding_method', in: 'query', schema: { type: 'string', enum: ['bank_account'] } },
            { name: 'amount', in: 'query', schema: { type: 'string' }, description: 'TRY amount (optional; the simulated transfer amount is used otherwise)' },
            { name: 'memo', in: 'query', schema: { type: 'string' } }, { name: 'memo_type', in: 'query', schema: { type: 'string', enum: ['text', 'id', 'hash'] } },
            { name: 'claimable_balance_supported', in: 'query', schema: { type: 'boolean' } },
            { name: 'on_change_callback', in: 'query', schema: { type: 'string' } }, { name: 'lang', in: 'query', schema: { type: 'string' } },
          ],
          responses: { 200: resp('Deposit instructions', ref('Sep6DepositResponse')), 400: resp('Bad request', { type: 'object', properties: { error: { type: 'string' } } }), 403: resp('authentication_required') },
        },
      },
      '/sep6/deposit-exchange': { get: { tags: ['SEP-6'], security: [{ Sep10Jwt: [] }], summary: 'Deposit TRY, receive USDC, optionally at a SEP-38 quote', parameters: [{ name: 'asset_code', in: 'query', required: true, schema: { type: 'string' } }, { name: 'source_asset', in: 'query', required: true, schema: { type: 'string', example: 'iso4217:TRY' } }, { name: 'destination_asset', in: 'query', required: true, schema: { type: 'string', example: 'stellar:USDC:G…' } }, { name: 'amount', in: 'query', required: true, schema: { type: 'string' } }, { name: 'quote_id', in: 'query', schema: { type: 'string' } }, { name: 'account', in: 'query', schema: { type: 'string' } }], responses: { 200: resp('Deposit instructions', ref('Sep6DepositResponse')) } } },
      '/sep6/withdraw': {
        get: {
          tags: ['SEP-6'], security: [{ Sep10Jwt: [] }], summary: 'Open a withdrawal: treasury account + memo id',
          description: 'Creates an off-ramp. Pay USDC to `account_id` with the returned memo (type id); the payment is detected on Horizon, sold at the locked rate and paid out (simulated FAST) to the wallet user\'s sandbox IBAN (or the IBAN sent via SEP-12).',
          parameters: [{ name: 'asset_code', in: 'query', required: true, schema: { type: 'string' } }, { name: 'funding_method', in: 'query', schema: { type: 'string', enum: ['bank_account'] } }, { name: 'type', in: 'query', schema: { type: 'string' }, description: 'Deprecated alias of funding_method' }, { name: 'amount', in: 'query', schema: { type: 'string' } }, { name: 'account', in: 'query', schema: { type: 'string' } }, { name: 'refund_memo', in: 'query', schema: { type: 'string' } }, { name: 'refund_memo_type', in: 'query', schema: { type: 'string' } }, { name: 'on_change_callback', in: 'query', schema: { type: 'string' } }],
          responses: { 200: resp('Withdraw instructions', ref('Sep6WithdrawResponse')), 403: resp('authentication_required') },
        },
      },
      '/sep6/withdraw-exchange': { get: { tags: ['SEP-6'], security: [{ Sep10Jwt: [] }], summary: 'Withdraw USDC, receive TRY, optionally at a SEP-38 quote', parameters: [{ name: 'asset_code', in: 'query', required: true, schema: { type: 'string' } }, { name: 'source_asset', in: 'query', required: true, schema: { type: 'string', example: 'stellar:USDC:G…' } }, { name: 'destination_asset', in: 'query', required: true, schema: { type: 'string', example: 'iso4217:TRY' } }, { name: 'amount', in: 'query', required: true, schema: { type: 'string' } }, { name: 'quote_id', in: 'query', schema: { type: 'string' } }], responses: { 200: resp('Withdraw instructions', ref('Sep6WithdrawResponse')) } } },
      '/sep6/transactions': { get: { tags: ['SEP-6'], security: [{ Sep10Jwt: [] }], summary: 'Transaction history for the authenticated account', parameters: [{ name: 'asset_code', in: 'query', required: true, schema: { type: 'string' } }, { name: 'kind', in: 'query', schema: { type: 'string', enum: ['deposit', 'withdrawal'] } }, { name: 'limit', in: 'query', schema: { type: 'integer' } }, { name: 'no_older_than', in: 'query', schema: { type: 'string', format: 'date-time' } }, { name: 'paging_id', in: 'query', schema: { type: 'string' } }], responses: { 200: resp('Transactions', { type: 'object', properties: { transactions: { type: 'array', items: ref('Sep6Transaction') } } }) } } },
      '/sep6/transaction': { get: { tags: ['SEP-6'], security: [{ Sep10Jwt: [] }], summary: 'One transaction', parameters: [{ name: 'id', in: 'query', schema: { type: 'string' } }, { name: 'stellar_transaction_id', in: 'query', schema: { type: 'string' } }, { name: 'external_transaction_id', in: 'query', schema: { type: 'string' } }], responses: { 200: resp('Transaction', { type: 'object', properties: { transaction: ref('Sep6Transaction') } }), 404: resp('Not found') } } },
      '/sep6/tx/{id}': { get: { tags: ['SEP-6', 'Sandbox'], security: [], summary: 'more_info_url page (HTML). For pending deposits it has the "simulate incoming TRY transfer" button.', parameters: [id('id')], responses: { 200: { description: 'HTML' } } } },
      '/sep6/tx/{id}/simulate-bank-transfer': { post: { tags: ['SEP-6', 'Sandbox'], security: [], summary: 'Sandbox bank: the TRY for this deposit arrived', description: 'Credits the wallet user, creates the on-ramp and pays real testnet USDC. The transaction id acts as the capability.', parameters: [id('id')], requestBody: json({ type: 'object', properties: { amount: dec('TRY amount; defaults to the deposit amount or 1000.00', '150.00') } }, false), responses: { 200: resp('Funded', { type: 'object', properties: { ok: { type: 'boolean' }, transaction: ref('Sep6Transaction') } }), 409: resp('Already funded') } } },
      '/sep12/customer': {
        get: { tags: ['SEP-12'], security: [{ Sep10Jwt: [] }], summary: 'KYC status (NEEDS_INFO until any PUT, then ACCEPTED)', parameters: [{ name: 'id', in: 'query', schema: { type: 'string' } }, { name: 'account', in: 'query', schema: { type: 'string' } }, { name: 'memo', in: 'query', schema: { type: 'string' } }, { name: 'type', in: 'query', schema: { type: 'string' } }], responses: { 200: resp('Customer', { type: 'object', properties: { id: { type: 'string' }, status: { type: 'string', enum: ['ACCEPTED', 'NEEDS_INFO'] }, fields: { type: 'object' }, provided_fields: { type: 'object' }, message: { type: 'string' } } }) } },
        put: { tags: ['SEP-12'], security: [{ Sep10Jwt: [] }], summary: 'Register / update the customer (all fields optional)', description: 'JSON, form or multipart. Accepted optional fields: first_name, last_name, email_address, bank_account_number (Turkish IBAN), bank_name. Identity numbers, dates of birth and documents are discarded, never stored.', responses: { 202: resp('Accepted', { type: 'object', properties: { id: { type: 'string' } } }) } },
      },
      '/sep12/customer/callback': { put: { tags: ['SEP-12'], security: [{ Sep10Jwt: [] }], summary: 'Register a KYC status callback URL (accepted, never fired: status is always ACCEPTED)', responses: { 200: resp('OK') } } },
      '/sep12/customer/{account}': { delete: { tags: ['SEP-12'], security: [{ Sep10Jwt: [] }], summary: 'Forget optional data for the account (or account+memo in the body)', parameters: [{ name: 'account', in: 'path', required: true, schema: { type: 'string' } }], responses: { 200: resp('Deleted'), 404: resp('Not yours / not found') } } },
      '/sep38/info': { get: { tags: ['SEP-38'], security: [], summary: 'Supported assets and delivery methods', responses: { 200: resp('Info') } } },
      '/sep38/prices': { get: { tags: ['SEP-38'], security: [], summary: 'Indicative prices for a sell asset', parameters: [{ name: 'sell_asset', in: 'query', required: true, schema: { type: 'string' } }, { name: 'sell_amount', in: 'query', required: true, schema: { type: 'string' } }], responses: { 200: resp('Prices') } } },
      '/sep38/price': { get: { tags: ['SEP-38'], security: [], summary: 'Indicative price for a pair', parameters: [{ name: 'sell_asset', in: 'query', required: true, schema: { type: 'string' } }, { name: 'buy_asset', in: 'query', required: true, schema: { type: 'string' } }, { name: 'sell_amount', in: 'query', schema: { type: 'string' } }, { name: 'buy_amount', in: 'query', schema: { type: 'string' } }, { name: 'context', in: 'query', schema: { type: 'string', enum: ['sep6'] } }], responses: { 200: resp('Price', ref('Sep38Price')) } } },
      '/sep38/quote': { post: { tags: ['SEP-38'], security: [{ Sep10Jwt: [] }], summary: 'Firm quote (default 15 min, max 1 h via expire_after)', requestBody: json({ type: 'object', required: ['sell_asset', 'buy_asset'], properties: { sell_asset: { type: 'string' }, buy_asset: { type: 'string' }, sell_amount: { type: 'string' }, buy_amount: { type: 'string' }, expire_after: { type: 'string', format: 'date-time' }, context: { type: 'string' } } }), responses: { 201: resp('Quote', ref('Sep38Quote')) } } },
      '/sep38/quote/{id}': { get: { tags: ['SEP-38'], security: [{ Sep10Jwt: [] }], summary: 'Fetch a quote', parameters: [id('id')], responses: { 200: resp('Quote', ref('Sep38Quote')), 404: resp('Unknown') } } },
    },
    components: {
      securitySchemes: {
        ...
        Sep10Jwt: { type: 'http', scheme: 'bearer', bearerFormat: 'JWT', description: 'SEP-10 token from POST /auth. Failures return HTTP 403 {"type":"authentication_required"}.' },
      },
      schemas: {
        ...
        Sep6DepositResponse: { type: 'object', properties: { id: { type: 'string', example: 'sep_…' }, how: { type: 'string' }, instructions: { type: 'object', description: 'SEP-9 fields: bank_name, bank_account_number (IBAN), external_transfer_memo (reference)', additionalProperties: { type: 'object', properties: { value: { type: 'string' }, description: { type: 'string' } } } }, eta: { type: 'integer' }, min_amount: { type: 'number' }, max_amount: { type: 'number' }, fee_percent: { type: 'number' }, extra_info: { type: 'object', properties: { message: { type: 'string' } } } } },
        Sep6WithdrawResponse: { type: 'object', properties: { id: { type: 'string' }, account_id: { type: 'string', description: 'Treasury account to pay' }, memo_type: { type: 'string', example: 'id' }, memo: { type: 'string' }, eta: { type: 'integer' }, min_amount: { type: 'number' }, fee_percent: { type: 'number' }, extra_info: { type: 'object' } } },
        Sep6Transaction: {
          type: 'object',
          properties: {
            id: { type: 'string' }, kind: { type: 'string', enum: ['deposit', 'withdrawal'] },
            status: { type: 'string', enum: ['incomplete', 'pending_user_transfer_start', 'pending_anchor', 'pending_stellar', 'pending_trust', 'pending_external', 'completed', 'error'] },
            status_eta: { type: 'integer', nullable: true }, more_info_url: { type: 'string' }, message: { type: 'string', nullable: true },
            amount_in: { type: 'string', nullable: true }, amount_in_asset: { type: 'string' }, amount_out: { type: 'string', nullable: true }, amount_out_asset: { type: 'string' }, amount_fee: { type: 'string', nullable: true }, amount_fee_asset: { type: 'string' }, fee_details: { type: 'object', nullable: true },
            quote_id: { type: 'string', nullable: true }, from: { type: 'string', nullable: true }, to: { type: 'string', nullable: true },
            deposit_memo: { type: 'string', nullable: true }, deposit_memo_type: { type: 'string', nullable: true }, withdraw_anchor_account: { type: 'string' }, withdraw_memo: { type: 'string', nullable: true }, withdraw_memo_type: { type: 'string' },
            started_at: { type: 'string', format: 'date-time' }, updated_at: { type: 'string', format: 'date-time' }, completed_at: { type: 'string', format: 'date-time', nullable: true }, user_action_required_by: { type: 'string', nullable: true },
            stellar_transaction_id: { type: 'string', nullable: true }, external_transaction_id: { type: 'string', nullable: true }, claimable_balance_id: { type: 'string', nullable: true }, instructions: { type: 'object', nullable: true }, refunded: { type: 'boolean' }, refunds: { type: 'object', nullable: true },
          },
        },
        Sep38Price: { type: 'object', properties: { total_price: { type: 'string' }, price: { type: 'string' }, sell_amount: { type: 'string' }, buy_amount: { type: 'string' }, fee: { type: 'object', properties: { total: { type: 'string' }, asset: { type: 'string' }, details: { type: 'array' } } } } },
        Sep38Quote: { type: 'object', properties: { id: { type: 'string' }, expires_at: { type: 'string' }, total_price: { type: 'string' }, price: { type: 'string' }, sell_asset: { type: 'string' }, sell_amount: { type: 'string' }, buy_asset: { type: 'string' }, buy_amount: { type: 'string' }, fee: { type: 'object' } } },
      },
    },
  };
}
